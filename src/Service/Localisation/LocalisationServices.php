...
class LocalisationServices
{
    ...
    public function __construct(GpsRepository $gpsRepository, EntityManagerInterface $em, Sessioninit $sessioninit){
     ...
     */
    public function newAdress($result, Website $website, $order): bool|Adresses
    {
        /** @var Adresses $adress */
        $template=$website->getTemplate();
        if($template->getSector()!=null){
            $sector=$template->getSector();
        }else{
            $sector=new Sectors();
        ...
        if($adress!=null) {
            $action=$this->adressor($adress, $result);
            $website->setLocality($action['gps']);
            return $action['adress'];
        }
        return false;
    }
     ...
     * @return array
     */
    public function adressor($adress, $result){
